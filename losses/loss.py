...
class Loss(torch.nn.Module):
    ...
    def __getitem__(self, item):
        if item in self.loss_mapper:
            return self.loss_mapper[item]
        else:
            raise KeyError(f"Loss '{item}' not found in the loss mapper.")

    def update_loss_weights(self, loss_log, epoch):
        if 'appearance' in self.loss_mapper and 'motion' in self.loss_mapper:
            self.appearance_loss_history.append(loss_log['appearance'])
            if epoch > 100:
                self.loss_weights['motion'] = self.motion_loss_weight * np.median(self.appearance_loss_history)
